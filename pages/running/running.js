// 全局命令
var util=require('../running/libs/utils.js')
...
Page({
  ...
  onLoad: function () {
    ...
    DB.where({
      ...
      success(res) {
        ...
        for(var b=1;b<7;b++)
        {
          k1[b]=Math.floor(Math.random()*29);
          for(var c=0;c<b;c++)
          {
            if(k1[b]==k1[c])
            {
  ...
  changeModel: function(){
    ...
    add({
      data:{
        datetime:that2.data.date,
        start_time:that2.data.start_time,
        end_time:that2.data.end_time,
